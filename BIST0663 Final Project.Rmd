...
###subject to fix
pred_test<-predict(ALZH_logistic,type='response',newdata = test_x)
...
```{r}
set.seed(12345)
ALZH.gbm<-ALZH_noID%>%select(-DoctorInCharge)%>%mutate(Diagnosis=as.numeric(as.character(Diagnosis)))
